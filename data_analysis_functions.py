...
def knn_classification_demo(df, target_column=None):
    st.header("K-Nearest Neighbors (KNN) Classification Demo")

    # If no target_column is provided, let user select
    if target_column is None:
        target_column = st.selectbox("Select the target (label) column:", df.columns)
    feature_columns = st.multiselect(
        "Select feature columns for KNN:",
        [col for col in df.columns if col != target_column],
        default=[col for col in df.columns if col != target_column][:2]
    )
    ...
    # KNN parameters
    n_neighbors = st.slider("Number of neighbors (k)", min_value=1, max_value=15, value=5)
    ...
    st.info("This demo shows how KNN can be used for classification. Try changing the features, target, and k value!")
